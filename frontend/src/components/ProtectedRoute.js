...
function ProtectedRoute({ children }) {
  ...
  useEffect(() => {
    if (localStorage.getItem('token')) {
      if (!user) {
        getUserData(); // Only fetch if user data is not already in Redux store
      } else {
        // If user is already in the store, you can directly set the menu
        if (user.isAdmin) {
          setMenu(adminMenu);
        } else {
          setMenu(userMenu);
        ...
        <div className='flex gap-2 h-100'>
          ...
          <div className='body'>
            <div className='header flex justify-between'>
              ...
              <h1 className='text-2xl text-white flex items-center'>Quiz Portal</h1>
              <div>
