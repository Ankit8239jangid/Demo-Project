...
export const editExam = async (payload, id) => {
  try {
    const response = await axiosInstance.put(`/api/exams/editExam/${id}`, payload); // Removed extra '/'
    return response.data;
  } catch (error) {
    return error.response.data;
  }
...
export const deleteQuestionFromExam = async (id) => { // Removed 'payload' argument
  try {
    const response = await axiosInstance.delete(`/api/exams/deleteQuestionFromExam/${id}`); // No body in DELETE request
    return response.data;
  } catch (error) {
    return error.response.data;
  }
