...
function AddEditExam() {
  ...
  const { id } = useParams();
  const [examData, setExamData] = useState(null); // Ensure the state is null initially
  const [showAddEditQuestionModal, setShowAddEditQuestionModal] = useState(false);
  const [selectedQuestion, setSelectedQuestion] = useState(null); // Initialize as null
  ...
  const deleteQuestionById = async (questionId) => {
    try {
      const reqPayload = { questionId: questionId };
      dispatch(ShowLoading());
    ...
    },
    {
      ...
      render: (text, record) => {
        return Object.keys(record.options).map((key) => {
          return <div key={key}>{key} : {record.options[key]}</div>;
        });
      },
    },
    {
